"""
Agentic RAG — LangGraph ReAct loop with wiki search tools.
...
Adaptive RAG lives in ``src.rag.query()``; this module is ``mode=agentic`` only.
"""
...
def _build_tools(
    ...
    def search_wiki(query: str) -> str:
        """Search the Witcher 3 wiki vector store. Pass a focused English query."""
    ...
    def refine_query(previous_query: str) -> str:
        """Rewrite a weak search query into a better English wiki search query."""
...
def query_agentic(
    ...
    """
    Run LangGraph agentic RAG: tool loop → grounded generate.

    Returns the same shape as ``rag.query()``, plus ``mode`` and ``agent_steps``.
    """
    start = time.time()
    llm = get_llm()
