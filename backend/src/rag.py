"""
Adaptive RAG pipeline — history-aware rewrite, score-gated retrieval, and
grounded answer generation.

This is the default query mode (``mode=adaptive``).
"""
...
def rewrite_standalone(
    ...
    """Turn the latest user message (+ history) into a standalone English search query."""
    result = invoke_text(
        llm,
        REWRITE_PROMPT,
...
def rewrite_for_search(llm: LLM, query: str) -> tuple[str, TokenUsage]:
    """Second-pass rewrite optimised for wiki vector search after a weak first hit."""
...
def retrieve_with_scores(
    ...
    """
    Similarity search returning documents and the best (lowest) distance score.
...
def retrieve_docs(
    ...
    """
    History-aware rewrite → retrieve.

    If the best distance exceeds ``retrieval_distance_threshold``, run a second
    search-oriented rewrite and retry.  Keeps the better of the two result sets.
    """
...
def query(
    ...
    """
    Run the adaptive RAG pipeline: rewrite → retrieve (score-gated retry) → generate.
    ...
    Returns:
        Dict with ``answer``, ``sources``, ``tokens``, ``response_time_ms``,
        ``mode``, and ``agent_steps``.
    """
    start = time.time()
